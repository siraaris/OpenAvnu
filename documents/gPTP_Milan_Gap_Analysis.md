# Milan gPTP Gap Analysis for `$HOME/src/gptp`

## Scope

This note assesses the local gPTP implementation in `$HOME/src/gptp` against the Milan 1.3 requirements that materially affect OpenAvnu integration:

- Milan 4.2.6 `gPTP`
- Milan 7.5 `gPTP as Media Clock Source`
- Milan 8.3.4 `gPTP` for redundancy

It also checks the current IPC contract between `$HOME/src/gptp` and `$HOME/src/OpenAvnu`, because Milan compliance for OpenAvnu depends on both sides being able to represent the required timing state.

## Executive Summary

The `$HOME/src/gptp` tree is a usable starting point for non-redundant Milan, but it is not Milan-ready as-is.

What is already useful:

- The non-automotive path has Milan-aligned default message intervals: Sync `125 ms`, Announce `1 s`, Pdelay `1 s` (`$HOME/src/gptp/common/ether_port.cpp:120-127`).
- The daemon default `priority1` is `248`, which matches Milan's grandmaster-capable default (`$HOME/src/gptp/linux/src/daemon_cl.cpp:145`, `$HOME/src/gptp/common/ieee1588clock.cpp:85-99`).
- `neighborPropDelayThresh` defaults to `800 ns` and is configurable (`$HOME/src/gptp/linux/src/daemon_cl.cpp:198-201`, `$HOME/src/gptp/gptp_cfg.ini:17-34`).
- gPTP frames are hard-wired to `transportSpecific = 1` on transmit and reject other values on receive (`$HOME/src/gptp/common/ptp_message.cpp:169-172`, `$HOME/src/gptp/common/ptp_message.cpp:608-619`).
- The implementation does handle the Milan multiple-Pdelay-response storm case by halting Pdelay for five minutes after three successive duplicates from different sources (`$HOME/src/gptp/common/ptp_message.cpp:1445-1464`).
- The IPC model exports most of the raw fields OpenAvnu needs for one AVB interface: current grandmaster ID/domain, interface clock identity, priority values, clock quality, intervals, port number, counters, `asCapable`, and port state (`$HOME/src/gptp/common/ipcdef.hpp:68-100`, `$HOME/src/gptp/linux/src/linux_hal_common.cpp:963-1055`).
...
Net: for non-redundant Milan, `$HOME/src/gptp` can probably be adapted with targeted fixes. For redundant Milan, the current IPC and process model need redesign.
...
See `$HOME/src/gptp/common/ether_port.cpp:116-127`.
...
The daemon default `priority1` is `248` (`$HOME/src/gptp/linux/src/daemon_cl.cpp:145`), and the clock object uses that value when constructed (`$HOME/src/gptp/linux/src/daemon_cl.cpp:410-412`, `$HOME/src/gptp/common/ieee1588clock.cpp:80-99`).
...
- duplicate detection and counter: `$HOME/src/gptp/common/ptp_message.cpp:1445-1464`
- five-minute restart timer: `$HOME/src/gptp/common/ptp_message.cpp:1460-1463`
...
See `$HOME/src/gptp/common/ipcdef.hpp:68-100` and `$HOME/src/gptp/linux/src/linux_hal_common.cpp:963-1055`.
...
- In Automotive Profile mode, the port is marked `asCapable` immediately in the constructor, before any Pdelay exchange (`$HOME/src/gptp/common/ether_port.cpp:104-116`).
- On Automotive Profile link-up, it is set `TRUE` again immediately (`$HOME/src/gptp/common/ether_port.cpp:427-429`).
- In the non-AP path, a single acceptable link delay causes `asCapable` to become `TRUE` immediately (`$HOME/src/gptp/common/ptp_message.cpp:1818-1830`).
...
See `$HOME/src/gptp/gptp_cfg.ini:2-34`.

The daemon parses and logs those values (`$HOME/src/gptp/linux/src/daemon_cl.cpp:434-446`), but:

- the clock is constructed before the config file is parsed (`$HOME/src/gptp/linux/src/daemon_cl.cpp:410-412`, `$HOME/src/gptp/linux/src/daemon_cl.cpp:432-468`)
...
- timeout multipliers defined here: `$HOME/src/gptp/common/common_port.hpp:48-50`
- sync receipt timer restarts here: `$HOME/src/gptp/common/ptp_message.cpp:1157-1162`, `$HOME/src/gptp/common/ether_port.cpp:384-387`, `$HOME/src/gptp/common/ether_port.cpp:454-457`, `$HOME/src/gptp/common/ether_port.cpp:511-514`, `$HOME/src/gptp/common/ether_port.cpp:706-709`
- announce receipt timer paths here: `$HOME/src/gptp/common/common_port.cpp:573-574`, `$HOME/src/gptp/common/ptp_message.cpp:881-884`, `$HOME/src/gptp/common/ether_port.cpp:419-424`
...
The repo explicitly documents Automotive Profile behavior, including BMCA disablement, signaling-based interval changes, and fixed GM/slave operation (`$HOME/src/gptp/README_AVNU_AP.txt:5-31`, `$HOME/src/gptp/README_AVNU_AP.txt:43-50`).

In code, the Automotive Profile path forces the port state to `MASTER` or `SLAVE` when `-V` is used (`$HOME/src/gptp/linux/src/daemon_cl.cpp:489-500`).
...
See `$HOME/src/gptp/common/ether_port.cpp:104-115`.
...
- it sets a single port index (`portInit.index = 1`) (`$HOME/src/gptp/linux/src/daemon_cl.cpp:423-425`)
- the producer shared memory name is fixed to `/ptp` (`$HOME/src/gptp/linux/src/linux_ipc.hpp:39-40`)
- the "group" argument changes ownership, not the shared memory namespace (`$HOME/src/gptp/linux/src/linux_hal_common.cpp:896-925`)
- the producer always unlinks that single shared memory object on shutdown (`$HOME/src/gptp/linux/src/linux_hal_common.cpp:891-893`)
...
This is technically an OpenAvnu-side issue, but it materially affects whether `$HOME/src/gptp` can be consumed correctly.

Producer side:

- shared memory size is `sizeof(gPtpTimeData) + sizeof(pthread_mutex_t)` (`$HOME/src/gptp/linux/src/linux_ipc.hpp:39`)
- producer struct includes counters, `asCapable`, port state, and process ID (`$HOME/src/gptp/common/ipcdef.hpp:94-100`)

OpenAvnu consumer side:

- still maps `SHM_NAME "/ptp"` (`$HOME/src/OpenAvnu/lib/common/avb_gptp.h:6-7`)
- uses an obsolete `SHM_SIZE` macro (`$HOME/src/OpenAvnu/lib/common/avb_gptp.h:6`)
- consumer struct stops at `port_number` and does not include the Linux-specific tail (`$HOME/src/OpenAvnu/lib/common/avb_gptp.h:11-36`)
- `gptpinit()` maps only that smaller size (`$HOME/src/OpenAvnu/lib/common/avb_gptp.c:24-37`)
- `gptpgetdata()` copies `sizeof(*td)` from shared memory (`$HOME/src/OpenAvnu/lib/common/avb_gptp.c:79-86`)
...
- Pdelay turnaround time is computed but not checked against a Milan limit or used to implement late-response handling (`$HOME/src/gptp/common/ptp_message.cpp:1327-1345`).
...
- `$HOME/src/gptp` is not the main blocker for non-redundant devices
- `$HOME/src/gptp` is a blocker for redundant devices because it is not exposed as two independent timing endpoints
...
For non-redundant Milan, `$HOME/src/gptp` is salvageable and already has several Milan-friendly defaults in the non-AP path.
